...
export default function ServicePage({ service, specialists, onBookAppointment }: ServicePageProps) {
  ...
  return (
    <main className="bg-slate-50 dark:bg-[#111827]">
      <section className="relative overflow-hidden bg-[#f7f0e5] py-20 md:py-24 border-b border-[#E1D8CC] dark:border-slate-800/50">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl text-[#333333] dark:text-[#F8FAFC] leading-tight">
                  {service.title}
                </h1>
            ...
            <div className="rounded-[2rem] border border-[#E1D8CC] dark:border-slate-800 bg-white dark:bg-[#111827]/90 p-6 shadow-xl">
              <div className="flex items-center justify-between border-b border-stone-200 dark:border-slate-800 pb-4">
                <div>
                  ...
                  <h2 className="mt-1 font-serif text-2xl text-[#333333] dark:text-[#F8FAFC]">What this program includes</h2>
                </div>
              ...
              <div className="mt-6 space-y-3">
                {service.bullets.map((bullet) => (
                  <div key={bullet} className="flex items-start gap-3 rounded-2xl bg-slate-50 dark:bg-[#111827] px-4 py-3">
      ...
      <section className="bg-white dark:bg-[#111827] py-20">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {serviceHighlights.map((item) => (
                <div key={item.title} className="flex gap-3 rounded-2xl border border-[#E1D8CC] dark:border-slate-800 bg-[#f7f0e5]/60 p-4">
          ...
          <div className="rounded-[2rem] border border-[#E1D8CC] dark:border-slate-800 bg-slate-50 dark:bg-[#111827] p-6 shadow-sm">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-[#111827] p-4">
                <h3 className="font-serif text-xl text-[#333333] dark:text-[#F8FAFC]">Adults & professionals</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">For those balancing stress, burnout, focus challenges, or emotional fatigue.</p>
              </div>
              <div className="rounded-2xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-[#111827] p-4">
                <h3 className="font-serif text-xl text-[#333333] dark:text-[#F8FAFC]">Couples & families</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">For supportive guidance through conflict, communication, and emotional connection.</p>
              </div>
              <div className="rounded-2xl border border-stone-200 dark:border-slate-800 bg-white dark:bg-[#111827] p-4 sm:col-span-2">
                <h3 className="font-serif text-xl text-[#333333] dark:text-[#F8FAFC]">Care that feels grounded</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">Each path is designed to feel practical, hopeful, and personalized from your first consultation onward.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-slate-50 dark:bg-[#111827] py-20">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
            {specialists.map((spec) => (
              <div key={spec.id} className="flex h-full flex-col justify-between rounded-[2rem] border border-stone-200 dark:border-slate-800 bg-white dark:bg-[#111827] p-6 shadow-sm">
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap justify-center gap-2">
                    {spec.specialties.map((item) => (
                      <span key={item} className="rounded-full border border-stone-200 dark:border-slate-800 bg-slate-50 dark:bg-[#111827] px-2.5 py-1 text-[10px] font-medium text-slate-600 dark:text-slate-300">
                        {item}
                      </span>
                    ))}
                  </div>
      ...
      <section className="bg-[#333333] dark:bg-brand-primary py-16">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 px-6 md:flex-row">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#C76B3D]">Ready when you are</p>
            <h2 className="mt-2 font-serif text-3xl text-[#f7f0e5] dark:text-[#333333]">Take the next step with Mentalist</h2>
          </div>
          <button
            onClick={onBookAppointment}
            className="rounded-full bg-[#C76B3D] px-6 py-3 text-sm font-semibold text-white transition-all duration-300 hover:bg-[#843519]"
